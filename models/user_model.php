...
<?php
     class User_Model
     {
        ...
        public function update_user($user_id,$user_email,$user_phone,$user_name, $user_address){
            $user_id = mysqli_real_escape_string($this->db->link,$user_id);
            $user_email = mysqli_real_escape_string($this->db->link,$user_email);
            $user_phone = mysqli_real_escape_string($this->db->link,$user_phone);
            $user_name = mysqli_real_escape_string($this->db->link,$user_name);
            $user_address = mysqli_real_escape_string($this->db->link,$user_address);
            if(empty($user_email)  || empty($user_phone)|| empty($user_name)|| empty($user_address) ) {
                $alert = "<span class = 'error'>Not empty</span>";
                return $alert;
            }
             else{
                $query = "UPDATE tbl_user SET user_email='$user_email',user_phone='$user_phone',user_name='$user_name',user_address='$user_address' 
                WHERE user_id = '$user_id'";
                
                 $result = $this->db->update($query);
                 if($result){
                    $alert = "<span class = 'success' >User update successfully</span>";
                return $alert;
                 } else{
                    $alert = "<span class = 'error'>User update not successfully</span>";
                return $alert;
                 }
            }
        }
        public function delete_user($id) {
            ...
            if($result){
                $alert = "<span class = 'success' >User delete successfully</span>";
            return $alert;
             } else{
                $alert = "<span class = 'error'>User delete not successfully</span>";
            return $alert;
             }
        }
    }
